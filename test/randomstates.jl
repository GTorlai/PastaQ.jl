...
@testset "randomstate: LPDO" begin
  ...
  # Real-valued with randpars
  ρ = randomstate(N;χ=χ,ξ=ξ,lpdo=true,complex=false)
  @test length(ρ) == N
  ...
  # Complex-valued with randpars
  ρ = randomstate(N;χ=χ,ξ=ξ,lpdo=true,complex=true)
  @test length(ρ) == N
  @test maxlinkdim(ρ.X) == χ 
  for j in 1:length(ρ)
  ...
  # Normalization
  ρ = randomstate(N;χ=χ,normalize=true,lpdo=true,complex=false)
  @test tr(ρ) ≈ 1
  ρ = randomstate(N;χ=χ,complex=true,normalize=true,lpdo=true)
  @test tr(ρ) ≈ 1
...
@testset "randomprocess: LPDO" begin
  ...
  # Real-valued with randpars
  ρ = randomprocess(N;χ=χ,ξ=ξ,mixed=true,complex=false)
  @test length(ρ) == N
  @test maxlinkdim(ρ.X) == χ 
  for j in 1:length(ρ)
    @test eltype(ρ.X[j]) == Float64
  ...
  # Complex-valued with randpars
  ρ = randomprocess(N;χ=χ,ξ=ξ,mixed=true,complex=true)
  @test length(ρ) == N
  @test maxlinkdim(ρ.X) == χ 
  for j in 1:length(ρ)
    @test eltype(ρ.X[j]) == Complex{Float64}
...
@testset "initialization given a process" begin
  ...
  ## 1. Given a LPDO
  ρ0 = randomprocess(N;mixed=true,χ=χ)
