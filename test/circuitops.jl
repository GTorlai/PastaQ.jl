...
@testset "apply gate: X" begin
  ...
  g = gate("X",site_ind)
  PastaQ.applygate!(psi,g)
  @test PastaQ.fullvector(psi) ≈ [0.,1.,0.,0.]
...
@testset "apply gate: Y" begin
  # Apply gate using gate informations
  psi = qubits(1)
  PastaQ.applygate!(psi,"Y",1)
  @test PastaQ.fullvector(psi) ≈ [0.,im]
  psi = qubits(1)
  PastaQ.applygate!(psi,"X",1)
  PastaQ.applygate!(psi,"Y",1)
  @test PastaQ.fullvector(psi) ≈ [-im,0.]
  ...
  g = gate("Y",site_ind)
  PastaQ.applygate!(psi,g)
  @test PastaQ.fullvector(psi) ≈ [0.,im,0.,0.]
...
@testset "apply gate: H" begin
  ...
  g = gate("H",site_ind)
  PastaQ.applygate!(psi,g)
  @test PastaQ.fullvector(psi) ≈ 1/sqrt(2.)*[1.,1.,0.,0.]
...
@testset "apply gate: Rn" begin
  ...
  λ = 2π * angles[3]
  psi = qubits(1)
  PastaQ.applygate!(psi,"Rn",1,θ=θ,ϕ=ϕ,λ=λ)
