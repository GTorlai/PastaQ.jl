...
@testset "rayleigh_quotient" begin
  ...
  function variational_circuit(θ⃗)
    N = length(θ⃗)
    return vcat(
      Rylayer(N, θ⃗), CXlayer(N), Rxlayer(N, θ⃗), Rylayer(N, θ⃗), CXlayer(N), Rxlayer(N, θ⃗)
    )
  end
...
@testset "fidelity optimization - Trotter circuit" begin
  ...
  import PastaQ: gate
  @eval function gate(::GateName"ZZ")
    return kron(gate("Z", SiteType("Qubit")), gate("Z", SiteType("Qubit")))
  end
