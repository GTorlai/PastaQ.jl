...
"""
    normalize!(L::LPDO; norm_per_site! = [])
    normalize!(ψ::MPS; norm_per_site! = [])
...
Passing a vector `v` as the keyword argument `norm_per_site!` will fill the vector with the normalization factor per site. For an MPS `ψ`, `prod(v) ≈ norm(ψ)`, while for an LPDO `L`, `prod(v) ≈ tr(L)`.
"""
function LinearAlgebra.normalize!(L::LPDO; norm_per_site! = [])
  N = length(L)
  resize!(norm_per_site!, N)
  # TODO: replace with:
  ...
  localZ = norm(blob)
  L.X[N] /= sqrt(localZ)
  norm_per_site![N] = sqrt(localZ)
  return L
end

function LinearAlgebra.normalize!(ψ::MPS; norm_per_site! = [])
  normalize!(LPDO(ψ); norm_per_site! = norm_per_site!)
  return ψ
end
