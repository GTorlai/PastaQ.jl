...
function printlattice!(p::Plots.Plot, code::SurfaceCode)
...
printlattice(code::SurfaceCode; kwargs...) =
  printlattice!(plot([0],[0],color=:black, legend=false, aspect_ratio=:equal,
    background_color=:transparent), code; kwargs...)
...
function printsyndrome!(p::Plots.Plot, s::NamedTuple, code::SurfaceCode)
  ...
  for e in charges
    p = plot!([code.Xcoord[e][1]],[code.Xcoord[e][2]],markersize=3*markersize,color=:red,marker=:star5, opacity=.75)
  end
  for f in fluxes
    p = plot!([code.Zcoord[f][1]],[code.Zcoord[f][2]],markersize=3*markersize,color=:blue,marker=:star5, opacity=.75)
  end
  #p = annotate!(0, 0, text("Ẑ", :red, :right, 10))
  return p
end

function printsyndrome(s::NamedTuple, code::SurfaceCode; kwargs...)
  p = printlattice(code)
  p = printsyndrome!(p, s, code; kwargs...)
  return p
end

function printpauli!(p::Plots.Plot, e::Vector{<:Array}, code::SurfaceCode)
