...
ITensors.dag(single_gate::Tuple{String,Union{Int,Tuple},NamedTuple}) = 
  (single_gate[1], single_gate[2], (single_gate[3]..., dag = true,))

ITensors.dag(layer::Vector{<:Any}) = 
  [dag(g) for g in reverse(layer)]
#ITensor.dag(circuit::Vector{<:Vector{<:Any}}) = 
#  Vector[dag(layer) for layer in reverse(circuit)]

function ITensors.dag(circuit::Vector{<:Vector{<:Any}})
  newcircuit = Vector[] 
