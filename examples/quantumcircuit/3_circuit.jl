...
for n in 1:nshots
  ...
  gate_tensors = vcat(circuit_tensors,meas_tensors)
  psi_out = runcircuit(psi,gate_tensors)
...
for n in 1:nshots
  ...
  gate_tensors = vcat(prep_tensors,circuit_tensors)

  meas_gates = makemeasurementgates(bases[n,:])
  meas_tensors = compilecircuit(psi,meas_gates)
  gate_tensors = vcat(gate_tensors,meas_tensors)
